...
let
  # Default configuration matching your config.json
  defaultSettings = {
    positionX = "right";
    positionY = "top";
    layer = "overlay";
    ...
    widget-config = {
      ...
      "buttons-grid" = {
        actions = [
          ...
          {
            label = "󰌾";
            command = "~/.config/hypr/scripts/lock-session.sh";
          }
          {
            label = "󰍃";
          ...
          {
            label = "󰖩";
            command = "~/.local/bin/shved/rofi-menus/wifi-menu.sh";
          }
          {
            label = "󰂯";
  ...
  # Default CSS style matching your provided style
  defaultStyle = ''
...
in {
  options.services.swaync = with lib; {
    ...
    settings = mkOption {
      ...
      description = ''
        Configuration for Sway Notification Center.
        See https://github.com/ErikReider/SwayNotificationCenter for options.
      '';
    };
    ...
    style = mkOption {
      ...
      description = "CSS style for Sway Notification Center.";
    };
  };

  config = lib.mkIf config.services.swaync.enable {
    environment.systemPackages = [ config.services.swaync.package ];

    xdg.configFile."swaync/config.json" = {
      text = builtins.toJSON config.services.swaync.settings;
    };

    xdg.configFile."swaync/style.css" = {
      text = config.services.swaync.style;
    };
  };
}
